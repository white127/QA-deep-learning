...
class RNN_Model(object):
    def _rnn_net(self, inputs, mask, embedding, keep_prob, batch_size, embed_dim, num_step, fw_cell, bw_cell):
          ...
          outputs = tf.reshape(tf.concat(outputs, axis=0), [num_step, batch_size, embed_dim * 2])
          self.outputs = tf.reshape(tf.concat(outputs, axis=0), [num_step, batch_size, embed_dim * 2])
          outputs = outputs[-1]
    ...
    def __init__(self, config, is_training=True):
        ...
        margin = tf.constant(np.full(self.batch_size, 0.05, dtype='float32'))
...
tf.flags.DEFINE_integer('evaluate_every',3000,'evaluate every')
